...
const FindResidence = () => {
  ...
  const handleFindResidences = async (event: React.FormEvent) => {
    ...
    if (amenities.length < 100 || details.length < 100) {
      window.alert('Please make sure amenities and details have at least 100 characters');
      return;
    }
  ...
  return (
    <>
      <label className="flex text-med justify-center h-[3rem] mt-1">
        ...
        <Dialog open={isResultOpen} as="div" className="relative z-10 focus:outline-none" onClose={closeResult}>
          <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
            <div className="flex min-h-full items-center justify-center p-4">
              <DialogPanel
                transition
                className="w-full max-w-md rounded-xl bg-white/5 p-6 backdrop-blur-2xl duration-300 ease-out data-[closed]:transform-[scale(95%)] data-[closed]:opacity-0"
              >
                <DialogTitle className="text-lg font-medium leading-6 text-white text-center">Ideal Residence Found!</DialogTitle>
